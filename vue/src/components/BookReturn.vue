  ...
  <div class="BookBorrowing">
    ...
    <el-card style="margin-top: 15px">
      <el-table :data="bookList" border style="width: 100%">
        ...
        <el-table-column prop="username" label="借阅者" width="180">
        </el-table-column>
        <el-table-column label="操作" width="180">
          ...
          <template slot-scope="scope">
            <el-button
              type="info"
              size="small"
              @click="openReturnBook(scope.row)"
              >归还图书</el-button
            >
          </template>
        </el-table-column>
        <el-table-column label="状态" width="180">
          ...
          <template slot-scope="scope">
            <el-button
              ...
              >暂无状态</el-button
            >
            <el-button type="warning" size="small" v-else
              >管理员已催还</el-button
            >
          </template>
        </el-table-column>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="OKReturnBook">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import { BookRturnSellctAPI, returnbookApi } from "@/request/api";
export default {
  computed: {
    //解构
    ...mapState(["username"]),
  ...
  methods: {
    ...
    async getBorrowingBooks() {
      let res = await BookRturnSellctAPI(this.paging);
      // console.log(res);
      if (res.status == 0) {
        // this.$message({
        //   message: res.message,
        ...
        this.bookList = res.data.map((book) => {
          const { time } = book;
    ...
    openReturnBook(row) {
      ...
      this.returnBook = row;
      this.returnBook.returntime = gettime();
      // console.log(this.returnBook);
    },
    async OKReturnBook() {
      let res = await returnbookApi(this.returnBook);
      if (res.status == 0) {
        //归还成功
        this.$message({
          message: res.message,
